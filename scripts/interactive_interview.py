...
# --- Configuration ---
# Set to False for a cleaner, chat-like experience.
# Set to True to see the detailed graph execution steps.
...
# Import the workflow and state objects
...
# In scripts/interactive_interview.py
...
def main():
    ...
    initial_state = {
        "session_id": str(session_id), "user_id": str(uuid.uuid4()),
        "initial_resume_text": initial_resume, "initial_job_description_text": initial_jd,
        "question_history": [], "resume_summary": None, "job_summary": None,
        "current_question": None, "personalization_profile": None, "current_rubric": None,
        "interview_plan": [], "final_report": None, "current_topic": None,
    }
    ...
    while True:
        ...
        graph.update_state(
            config,
            {"current_question": current_question_obj.model_dump(), "current_rubric": {}},
        )
        ...
        # --- THIS IS THE NEW, IMPROVED OUTPUT SECTION ---
        ...
        if last_turn.evals and last_turn.evals.get("canonical"):
            score = last_turn.evals["canonical"].get("final_score", "N/A")
